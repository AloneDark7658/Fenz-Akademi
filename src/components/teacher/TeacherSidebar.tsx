...
const NAV_ITEMS = [
  { href: "/teacher", label: "Genel Bakış", icon: LayoutDashboard, exact: true },
  { href: "/teacher/lessons", label: "Ders Yönetimi", icon: Video },
  { href: "/teacher/questions", label: "Soru Havuzu", icon: BookOpen },
...
export function TeacherSidebar() {
  ...
  return (
    <aside className="flex flex-col h-full w-64 bg-edu-navy border-r border-white/10">
      {/* Logo */}
      <div className="flex items-center gap-3 px-6 py-5 border-b border-white/10">
        <div className="bg-edu-cyan/10 border border-edu-cyan/30 rounded-xl p-2">
          <Rocket className="w-5 h-5 text-edu-cyan" />
        </div>
        <div>
          <p className="font-black text-white text-base leading-tight">Fenz Akademi</p>
          <p className="text-edu-cyan/70 text-xs font-medium">Öğretmen Paneli</p>
        </div>
      </div>
